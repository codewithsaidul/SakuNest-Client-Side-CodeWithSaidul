...
const MyBookings = () => {
    ...
    const handleBookingCancel = async(id,status) => {
        
        try {
            const { data } = await axios.patch(`${import.meta.env.VITE_API_URL}/bookRoom/${id}`, { status})
            console.log(data)
...

                                            {
                                                myBookings.map(booking =>
                                                    <tr key={booking._id}>
                                                        ...
                                                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{new Date(booking.bookingTime).toLocaleDateString()}</td>
                                                        ...
                                                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                            <div className="flex items-center gap-x-6">
                                                                <button onClick={() => handleBookingCancel(booking._id, "Cancelled")} disabled={booking.status === "Cancelled"} className="disabled:cursor-not-allowed text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
