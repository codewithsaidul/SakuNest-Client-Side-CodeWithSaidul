...
const MyBookings = () => {
    ...
    const handleBookingCancel = async(id, bookTime, status) => {
        ...
        try {
            ...
            const bookingTime = moment(bookTime).utcOffset('+06.00');
            const currentTime = moment().utcOffset('+06.00')
            const difference = currentTime.diff(bookingTime, 'date')
...

                                            {
                                                myBookings.map(booking =>
                                                    <tr key={booking._id}>
                                                        ...
                                                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{new Date(booking.bookibookingDatengTime).toLocaleDateString()}</td>
                                                        ...
                                                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                            <div className="flex items-center gap-x-6">
                                                                <button onClick={() => handleBookingCancel(booking._id, booking.bookingTime, "Cancelled")} disabled={booking.status === "Cancelled"} className="disabled:cursor-not-allowed text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
